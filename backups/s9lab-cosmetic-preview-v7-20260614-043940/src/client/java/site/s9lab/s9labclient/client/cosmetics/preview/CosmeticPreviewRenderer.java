...
/**
 * Non-mutating GUI entity renderer with 360-degree yaw
 * and explicit preview poses.
 */
public final class CosmeticPreviewRenderer {
    ...
    public static void draw(
        ...
        try {
            ...
            EntityRenderState state =
                    manager.getAndUpdateRenderState(entity, 1.0F);

            if (tryOn && cosmetic != null) {
                int stateKey = System.identityHashCode(state);

                CosmeticPreviewContext.bindState(
                        stateKey,
                        cosmetic,
                        hideBasePlayer
                );
            ...
            if (state instanceof LivingEntityRenderState living) {
                ...
                applyPose(
                        living,
                        pose == null ? PreviewPose.IDLE : pose
                );
            }
            ...
            Quaternionf pitchRotation = new Quaternionf()
                    .rotateX((float) Math.toRadians(-pitch * 0.35F));
    ...
    private static void applyPose(
        ...
        switch (pose) {
            case WALK -> {
                state.limbSwingAnimationProgress =
                        (System.currentTimeMillis() % 2400L) / 110.0F;
