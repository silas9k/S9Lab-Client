    ...
    private void renderCosmeticCard(DrawContext context, Cosmetic cosmetic, int x, int y, int width, int height, int mouseX, int mouseY, int accent) {
        ...
        if (client.player != null) {
            float cardYaw = switch (cosmetic.type()) {
                case CAPE, WINGS, SHOULDER -> 0.0F;
                case HAT, HALO, BANDANA -> 18.0F;
                default -> 145.0F;
            };
            int cardScale = Math.max(28, Math.min(62, Math.min(previewW, previewH) / 2));
            boolean hideBasePlayer = cosmetic.type() != CosmeticType.EMOTE
                    && cosmetic.type() != CosmeticType.GLINT;
            CosmeticPreviewRenderer.draw(
                    context, client.player, cosmetic, true, PreviewPose.IDLE,
                    previewX, previewY, previewX + previewW, previewY + previewH,
                    cardScale, cardYaw, 5.0F, hideBasePlayer
            );
        } else {
