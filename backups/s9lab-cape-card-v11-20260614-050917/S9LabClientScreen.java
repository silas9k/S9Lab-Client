    ...
    private void renderCosmeticCard(DrawContext context, Cosmetic cosmetic, int x, int y, int width, int height, int mouseX, int mouseY, int accent) {
        ...
        if (client.player != null) {
            float cardYaw = switch (cosmetic.type()) {
                // PlayerCapeModel is one-sided. The card must use the same
                // rear-facing orientation as the working large preview,
                // otherwise the cape is back-face culled and the card looks empty.
                case CAPE -> 180.0F;
                case WINGS -> 0.0F;
                default -> 180.0F;
            };
            BaseVisibility cardVisibility = switch (cosmetic.type()) {
                case CAPE, WINGS -> BaseVisibility.HIDDEN;
