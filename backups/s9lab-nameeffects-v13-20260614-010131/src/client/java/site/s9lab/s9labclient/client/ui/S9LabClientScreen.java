...
public class S9LabClientScreen extends ResponsiveScreen {
    ...
    private void renderNameEffectSettings(
        ...
        context.drawTextWithShadow(textRenderer, Text.literal("< S9C+ NAMETAG STYLE"), x + 14, y + 14, WHITE);
        context.drawTextWithShadow(textRenderer,
                Text.literal("Choose up to 3 effects. Unsupported combinations use the first animation."),
                x + 14, y + 31, MUTED);
        ...
        context.drawTextWithShadow(textRenderer, Text.literal(preview), x + 14, y + 52, previewColor);
        String slots = selected.isEmpty() ? "NO EFFECT" : String.join(" + ", selected.stream().map(S9TextEffects::displayName).toList());
        context.drawTextWithShadow(textRenderer, Text.literal(TextLayout.ellipsize(textRenderer, slots, width - 180)), x + 14, y + 68, DIM);
        renderSquareButton(context, x + width - 92, y + 47, 76, 26, "CLEAR", false, mouseX, mouseY, accent);

        int sectionY = y + 98;
        context.drawTextWithShadow(textRenderer, Text.literal("— ANIMATIONS —"), x + 14, sectionY, DIM);
        context.fill(x + 125, sectionY + 5, x + width - 14, sectionY + 6, 0x334A5160);
        sectionY += 20;
        int afterAnimations = renderNameEffectGrid(context, module, S9TextEffects.ANIMATION_IDS, x + 14, sectionY, width - 28, mouseX, mouseY, accent);

        int colorTitleY = afterAnimations + 12;
        context.drawTextWithShadow(textRenderer, Text.literal("— COLORS —"), x + 14, colorTitleY, DIM);
        context.fill(x + 92, colorTitleY + 5, x + width - 14, colorTitleY + 6, 0x334A5160);
        int afterColors = renderNameEffectGrid(context, module, S9TextEffects.COLOR_IDS, x + 14, colorTitleY + 20, width - 28, mouseX, mouseY, accent);

        int toggleY = afterColors + 14;
        renderNameEffectToggle(context, x + 14, toggleY, width - 28, "RENDER S9C+ EFFECT ON MY NAME", module.plusNameEffectsEnabled(), mouseX, mouseY, accent);
        renderNameEffectToggle(context, x + 14, toggleY + 34, width - 28, "SHOW OTHER PLAYERS' S9C+ EFFECTS", module.showOtherPlayersNameEffects(), mouseX, mouseY, accent);
    }
    ...
    private int renderNameEffectGrid(
        ...
        int columns = width >= 680 ? 4 : width >= 470 ? 3 : 2;
        int gap = 10;
        int buttonW = Math.max(82, (width - gap * (columns - 1)) / columns);
        int buttonH = 34;
        for (int index = 0; index < ids.size(); index++) {
            ...
            boolean selected = module.isEffectSelected(id);
            boolean hovered = inside(mouseX, mouseY, bx, by, buttonW, buttonH);
            int fill = selected ? 0x552D6DFF : hovered ? 0x332D6DFF : 0x6611161F;
            context.fill(bx, by, bx + buttonW, by + buttonH, fill);
            outline(context, bx, by, buttonW, buttonH, 0, selected ? accent : hovered ? 0xAA6B8EFF : 0x664A5160);
            int labelColor = selected ? S9TextEffects.previewColor(id) : hovered ? WHITE : MUTED;
            context.drawCenteredTextWithShadow(textRenderer,
                    Text.literal(TextLayout.ellipsize(textRenderer, S9TextEffects.displayName(id), buttonW - 12)),
                    bx + buttonW / 2, by + 12, labelColor);
            if (selected) {
                context.drawTextWithShadow(textRenderer, Text.literal("✓"), bx + buttonW - 14, by + 4, WHITE);
            }
        }
    ...
    private void renderNameEffectToggle(
        ...
        context.fill(x, y, x + width, y + 28, hovered ? 0x331E2938 : 0x2211161F);
        context.drawTextWithShadow(textRenderer, Text.literal(label), x + 8, y + 9, enabled ? WHITE : MUTED);
        int bx = x + width - 24;
        context.fill(bx, y + 5, bx + 18, y + 23, enabled ? 0x663A6FFF : 0x55252C38);
        outline(context, bx, y + 5, 18, 18, 0, enabled ? accent : 0x668A93A6);
        if (enabled) context.drawCenteredTextWithShadow(textRenderer, Text.literal("✓"), bx + 9, y + 8, WHITE);
    }
    ...
    private boolean handleNameEffectSettingsClick(
        ...
        int y = parts.contentY + 12;
        int width = parts.width - 20;
        if (inside(mouseX, mouseY, x + 6, y + 4, 180, 34)) {
            moduleDetailsOpen = false;
            return true;
        }
        if (inside(mouseX, mouseY, x + width - 92, y + 47, 76, 26)) {
            module.clearEffects();
            S9LabClientClient.getConfigManager().save();
            return true;
        }
        ...
        int animationY = y + 118;
        String clicked = clickedNameEffect(S9TextEffects.ANIMATION_IDS, gridX, animationY, gridWidth, mouseX, mouseY);
        int animationBottom = nameEffectGridBottom(S9TextEffects.ANIMATION_IDS, animationY, gridWidth);
        int colorY = animationBottom + 32;
        if (clicked == null) {
            clicked = clickedNameEffect(S9TextEffects.COLOR_IDS, gridX, colorY, gridWidth, mouseX, mouseY);
        }
        if (clicked != null) {
        ...
        int colorBottom = nameEffectGridBottom(S9TextEffects.COLOR_IDS, colorY, gridWidth);
        int toggleY = colorBottom + 14;
        if (inside(mouseX, mouseY, gridX, toggleY, gridWidth, 28)) {
            module.setPlusNameEffectsEnabled(!module.plusNameEffectsEnabled());
            S9LabClientClient.getConfigManager().save();
            return true;
        }
        if (inside(mouseX, mouseY, gridX, toggleY + 34, gridWidth, 28)) {
