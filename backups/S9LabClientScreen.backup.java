...
context.drawTexture(
    ...
    private void renderCosmetics(DrawContext context, Layout layout, int mouseX, int mouseY, ClientTheme theme) {
        ...
        int previewW = layout.bodyWidth() >= 570 ? 180 : 0;
        int gridW = layout.bodyWidth() - previewW - (previewW > 0 ? 12 : 0);
        Grid grid = grid(gridW, layout.contentHeight(), 126, 72, 4);
        ...
        for (int i = 0; i < cosmetics.size(); i++) {
            ...
            int cardY = baseY + row * (grid.cardH + grid.gap);
            if (cardY + grid.cardH < y || cardY > y + layout.contentHeight()) {
                continue;
            }
            renderCosmeticCard(context, cosmetic, cardX, cardY, grid.cardW, grid.cardH, mouseX, mouseY, theme);
        }
        if (previewW > 0) {
            renderCosmeticPreview(context, x + gridW + 12, y, previewW, layout.contentHeight(), theme);
        }
    }
    ...
    private boolean handleCosmeticClick(Layout layout, int mouseX, int mouseY) {
        ...
        if (selectedCosmetic != null && previewW > 0) {
            int previewX = x + gridW + 12;
            if (previewBounds(layout).contains(mouseX, mouseY) && !inside(mouseX, mouseY, previewX + 12, y + layout.contentHeight() - 30, previewW - 24, 20)) {
                previewDragging = true;
                return true;
            }
            if (inside(mouseX, mouseY, previewX + 12, y + layout.contentHeight() - 30, previewW - 24, 20)) {
    ...
    private Rect previewBounds(Layout layout) {
        if (layout.bodyWidth() < 570) {
            return Rect.empty();
        }
        int previewW = 180;
        int x = layout.bodyX() + layout.bodyWidth() - previewW;
        int y = layout.contentY();
        return new Rect(x, y, previewW, layout.contentHeight());
    }
    ...
    private int maxScroll(Layout layout) {
        int content = switch (selectedTab) {
            ...
            case COSMETICS, SHOP -> {
                int previewW = layout.bodyWidth() >= 570 ? 180 : 0;
                Grid grid = grid(layout.bodyWidth() - previewW - (previewW > 0 ? 12 : 0), layout.contentHeight(), 126, 72, 4);
