...
public class ScreenshotGalleryScreen extends ResponsiveScreen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        PremiumRender.card(context, layout.x, layout.y, layout.width, layout.height, theme.radius(), theme.panelColor(), theme.accentColor());
    ...
    private void drawHeader(DrawContext context, Layout layout, ClientTheme theme) {
        ...
        PremiumRender.card(context, pillX, pillY, 108, 17, theme.radius(), 0x66141B2A, 0x667CFFB2);
        context.drawCenteredTextWithShadow(this.textRenderer, Text.literal("PNG previews"), pillX + 54, pillY + 5, 0xFF7CFFB2);
    }
    ...
    private void drawScreenshotCard(DrawContext context, ScreenshotMetadata metadata, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        int border = hovered ? theme.accentColor() : theme.borderColor();
        int background = hovered ? theme.cardHoverColor() : theme.cardColor();
        PremiumRender.card(context, x, y, width, height, theme.radius(), background, border);

        int imageX = x + 8;
        int imageY = y + 8;
        int imageW = width - 16;
        int imageH = 68;
        PremiumRender.roundedRect(context, imageX, imageY, imageW, imageH, theme.radius(), 0xFF070A10);
    ...
    private void drawSearch(DrawContext context, int x, int y, int width, int mouseX, int mouseY) {
        ...
        PremiumRender.card(context, x, y, width, 24, theme.radius(), theme.cardColor(), active ? theme.accentColor() : theme.borderColor());
    ...
    private void drawEmptyState(DrawContext context, int gridY, int gridH, ClientTheme theme) {
        ...
        PremiumRender.card(context, boxX, boxY, boxW, 76, theme.radius(), 0xAA0A0D15, theme.borderColor());
    ...
    private void drawPreviewModal(DrawContext context, int mouseX, int mouseY) {
        ...
        PremiumRender.card(context, x, y, modalW, modalH, theme.radius(), theme.panelColor(), theme.accentColor());
        ...
        if (!status.isBlank()) {
            context.drawTextWithShadow(this.textRenderer, Text.literal(status), x + 18, y + modalH - 50, 0xFF7CFFB2);
        }
    }
