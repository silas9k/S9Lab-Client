...
public class S9LabClientScreen extends ResponsiveScreen {
    ...
    private void renderHeader(DrawContext context, Layout layout, int mouseX, int mouseY, int accent) {
        ...
        for (ClientTab tab : new ClientTab[]{ClientTab.MODS, ClientTab.SETTINGS, ClientTab.COSMETICS}) {
        ...
        rect(context, coinsBoxX + 9, y + 4, 14, 14, 7, ClientTheme.withAlpha(accent, 230));
        context.drawCenteredTextWithShadow(textRenderer, Text.literal("S"), coinsBoxX + 16, y + 8, WHITE);
        context.drawTextWithShadow(textRenderer, Text.literal("0"), coinsBoxX + 33, y + 7, WHITE);
    ...
    private void renderModuleCard(DrawContext context, Module module, int x, int y, int width, int height, int mouseX, int mouseY, int accent) {
        ...
        // TODO: add custom SVG/texture module icons here later.
    ...
    private void renderSettingsPage(DrawContext context, Layout layout, int mouseX, int mouseY, int accent) {
        ...
        int editorX = x + width - 132;
        boolean editorHover = inside(mouseX, mouseY, editorX, y, 124, 24);
        rect(context, editorX, y, 124, 24, 4, editorHover ? ClientTheme.withAlpha(accent, 180) : 0xFF22262D);
        outline(context, editorX, y, 124, 24, 4, editorHover ? accent : LINE_SOFT);
        context.drawCenteredTextWithShadow(textRenderer, Text.literal("HUD Editor"), editorX + 62, y + 8, editorHover ? WHITE : TEXT);
    ...
    private void renderCosmeticCard(DrawContext context, Cosmetic cosmetic, int x, int y, int width, int height, int mouseX, int mouseY, int accent) {
        ...
        outline(context, x, y, width, height, 8, selected || equipped ? ClientTheme.withAlpha(accent, 235) : hovered ? 0xAA45516B : LINE_SOFT);
        if (equipped) {
        ...
        // TODO: render clean cosmetic thumbnail texture here when cape/wing texture UVs are fixed.
        ...
        context.drawCenteredTextWithShadow(textRenderer, Text.literal(cosmeticRarity(cosmetic)), x + width / 2, y + height - 17, rarityColor(cosmetic));
    }
    ...
    private void renderCosmeticPreview(DrawContext context, int x, int y, int width, int height, int accent) {
        ...
        context.drawTextWithShadow(textRenderer, Text.literal("3D preview disabled for now"), x + 16, y + 30, DIM);

        int boxY = y + 58;
        int boxH = height - 116;
        rect(context, x + 14, boxY, width - 28, boxH, 12, 0x68101520);
        outline(context, x + 14, boxY, width - 28, boxH, 12, 0x22384255);
        context.drawCenteredTextWithShadow(textRenderer, Text.literal("PREVIEW"), x + width / 2, boxY + boxH / 2 - 12, MUTED);
        context.drawCenteredTextWithShadow(textRenderer, Text.literal("// TODO"), x + width / 2, boxY + boxH / 2 + 4, DIM);
        ...
        rect(context, x + 16, buttonY, width - 32, 28, 9, cosmetic == null ? 0x55151A25 : ClientTheme.withAlpha(accent, 210));
    ...
    private boolean handleSettingsPageClick(Layout layout, int mouseX, int mouseY) {
        ...
        int headerX = layout.x + 20;
        int headerY = layout.bodyY() + 10;
        int headerW = layout.width - 40;
        int editorX = headerX + headerW - 132;
        if (inside(mouseX, mouseY, editorX, headerY, 124, 24)) {
