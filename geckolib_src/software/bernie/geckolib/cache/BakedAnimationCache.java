 ...
 */
public record BakedAnimationCache(Map<class_2960, BakedAnimations> cache) {
    /**
     ...
     */
    public @Nullable Animation getAnimation(class_2960 animationFile, class_2960[] fallbackFiles, String animationName) {
        BakedAnimations animations = null;

        for (int i = -1; i < fallbackFiles.length; i++) {
            class_2960 path = i == -1 ? animationFile : fallbackFiles[i];
            animations = this.cache.get(path);

            if (animations == null) {
                class_2960 strippedPath = stripLegacyPath(path);
