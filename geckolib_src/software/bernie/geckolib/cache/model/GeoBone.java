...
public abstract class GeoBone {
     ...
     */
    public abstract <R extends GeoRenderState> void render(RenderPassInfo<R> renderPassInfo, class_4587 poseStack, class_4588 vertexConsumer, int packedLight, int packedOverlay, int renderColor);
    ...
    public void translateToPivotPoint(class_4587 poseStack) {
        poseStack.method_46416(pivotX() / 16f, pivotY() / 16f, pivotZ() / 16f);
    }
    ...
    public void updateBonePositionListeners(class_4587 poseStack, RenderPassInfo<?> renderPassInfo) {
        if (this.positionListeners != null) {
            final Matrix4f bonePose = new Matrix4f(poseStack.method_23760().method_23761());
            ...
            final class_243 position = renderPassInfo.renderState().getGeckolibData(DataTickets.POSITION);
            final Matrix4f worldPose = position == null ? null : RenderUtil.addPosToMatrix(new Matrix4f(localPose), position);
            final class_243 localPos = RenderUtil.renderPoseToPosition(localPose, 1, 1, 1);
            final class_243 modelPos = RenderUtil.renderPoseToPosition(modelPose, -16, 16, 16);
            final class_243 worldPos = worldPose == null ? null : RenderUtil.renderPoseToPosition(worldPose, 1, 1, 1);
