 ...
 * <p>
 * Due to the way Mojang handles {@link class_4588 buffers}, and for safety; this layer only supports one bone at a time.
...
public class CustomBoneTextureGeoLayer<T extends GeoAnimatable, O, R extends GeoRenderState> extends GeoRenderLayer<T, O, R> {
    ...
    protected void renderBone(RenderPassInfo<R> renderPassInfo, GeoBone bone, class_11659 renderTasks) {
        ...
        class_2960 boneTexture = getTextureResource(renderState);
        ...
        class_1921 renderType = getRenderType(renderState, boneTexture);
     ...
     * @see GeoCube#render(class_4587, class_4588, int, int, int)
     */
    @ApiStatus.Internal
     ...
     * @see GeoQuad#render(Matrix4f, Vector3f, class_4588, int, int, int)
     */
    @ApiStatus.Internal
    protected void renderQuad(GeoQuad quad, Matrix4f pose, Vector3f normal, class_4588 vertexConsumer, int packedLight, int packedOverlay, int renderColor, float widthRatio, float heightRatio) {
        for (GeoVertex vertex : quad.vertices()) {
            ...
            vertexConsumer.method_23919(vector4f.x(), vector4f.y(), vector4f.z(), renderColor, vertex.texU() * widthRatio,
