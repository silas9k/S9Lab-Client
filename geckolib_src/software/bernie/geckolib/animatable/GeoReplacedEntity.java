...
public interface GeoReplacedEntity extends SingletonGeoAnimatable {
	...
	default <D> @Nullable D getAnimData(class_1297 entity, SerializableDataTicket<D> dataTicket) {
		return getAnimatableInstanceCache().getManagerForId(entity.method_5628()).getAnimatableData(dataTicket);
	}
	...
	default <D> void setAnimData(class_1297 relatedEntity, SerializableDataTicket<D> dataTicket, D data) {
		if (relatedEntity.method_73183().method_8608()) {
			getAnimatableInstanceCache().getManagerForId(relatedEntity.method_5628()).setAnimatableData(dataTicket, data);
		}
		else {
	...
	default void triggerAnim(class_1297 relatedEntity, @Nullable String controllerName, String animName) {
		if (relatedEntity.method_73183().method_8608()) {
			AnimatableManager<GeoAnimatable> animatableManager = getAnimatableInstanceCache().getManagerForId(relatedEntity.method_5628());
	 ...
	 */
	@ApiStatus.NonExtendable
	default void stopTriggeredAnim(class_1297 relatedEntity, @Nullable String controllerName, @Nullable String animName) {
		if (relatedEntity.method_73183().method_8608()) {
			AnimatableManager<GeoAnimatable> animatableManager = getAnimatableInstanceCache().getManagerForId(relatedEntity.method_5628());
