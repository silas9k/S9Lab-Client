...
public class HashedStackMixin {
    ...
    public boolean geckolib$allowLazyStackIdParity(class_10936 patchMap, class_9326 componentPatch, class_10936.class_10937 hasher, Operation<Boolean> original) {
        if (!original.call(patchMap, componentPatch, hasher))
            return false;

        class_9331<Long> componentType = GeckoLibConstants.STACK_ANIMATABLE_ID_COMPONENT.get();
        int remoteStackHashedId = patchMap.comp_3874().getOrDefault(componentType, Integer.MIN_VALUE);
        int localStackHashedId = Optional.ofNullable(componentPatch.method_57845(componentType))
                .map(optional ->
                             optional.map(value -> hasher.apply(new class_9336<>(componentType, (long)value)))
