...
public final class BakedAnimationsAdapter {
	...
	private static Animation bakeAnimation(String name, JsonObject animationObj, JsonDeserializationContext context) throws CompoundException {
		double length = animationObj.has("animation_length") ? class_3518.method_34927(animationObj, "animation_length") : -1;
		LoopType loopType = LoopType.fromJson(animationObj.get("loop"));
		BoneAnimation[] boneAnimations = bakeBoneAnimations(class_3518.method_15281(animationObj, "bones", new JsonObject()));
	...
	private static KeyframeStack buildKeyframeStack(List<DoubleObjectPair<JsonElement>> entries, boolean isForRotation) throws CompoundException {
		...
		for (DoubleObjectPair<JsonElement> entry : entries) {
			...
			JsonArray keyFrameVector = element instanceof JsonArray array ? array : class_3518.method_15261(element.getAsJsonObject(), "vector");
			...
			List<MathValue> easingArgs = entryObj != null && entryObj.has("easingArgs") ?
										 JsonUtil.jsonArrayToList(class_3518.method_15261(entryObj, "easingArgs"), ele -> new Constant(ele.getAsDouble())) :
										 new ObjectArrayList<>();
