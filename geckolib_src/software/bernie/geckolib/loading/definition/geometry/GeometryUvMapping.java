 ...
 */
public record GeometryUvMapping(EnumMap<class_2350, GeometryUvMappingDetails> uvFaces) {
    /**
    ...
    public static JsonDeserializer<GeometryUvMapping> gsonDeserializer() throws JsonParseException {
        return (json, type, context) -> {
            ...
            final EnumMap<class_2350, GeometryUvMappingDetails> uvFaces = new EnumMap<>(class_2350.class);

            for (Map.Entry<String, JsonElement> face : obj.entrySet()) {
                final class_2350 direction = class_2350.method_10168(face.getKey());

                if (direction == null)
                    throw new JsonParseException("Error while parsing UV values. Expected Direction value: (" + Arrays.toString(class_2350.values()) + "), found " + face.getKey());
